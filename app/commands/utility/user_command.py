...
class UtilityUser(commands.Cog):
    ...
    async def user(self, ctx: ApplicationContext, user: Member = None):
        ...
        embed.description = await t('User information' if user_in_guild else 'User information without join date', ctx, [user.id, f'<t:{int(user.created_at.timestamp())}:F>', f'{ f"<t:{int(user.joined_at.timestamp())}:F>" if user_in_guild else None}'])
