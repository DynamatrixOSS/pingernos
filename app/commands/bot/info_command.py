...
class BotInfo(commands.Cog):
    ...
    async def info(self, ctx: ApplicationContext):
        ...
        information_embed.set_author(
            name=await t('Dynamatrix OSS by Dynamatrix', ctx),
            icon_url=self.bot.user.avatar.url
        )
        information_embed.set_footer(text=await t('Response time', ctx, [round(self.bot.latency * 1000)]))
        information_embed.description = await t(
            'is a bot developed by Dynamatrix OSS',
            ctx,
            parameters=[self.bot.user.mention, self.bot.user.display_name]
        )
